...
//Check if Rider ID exist then get riderid else me set riderid to null
...
if(mysqli_affected_rows($link))
{
 ...
 $result=mysqli_query($link,"insert into tbl_orders(OrderType,OrderUserId,User_Subsid,OrderTotalWeight,Order_PickDate,Order_Picktime,OrderDate,OrderStatusId,OrderDeliveryType,Order_Via,OrderShipName,OrderEmail,OrderPhone,OrderShipAddress,CreatedBy,RiderId)
  values('$ord_type','$uid',$user_subscription_id,$order_total_weight,'$pickdate','$picktime',NOW(),'$order_status_id','$delivery_type','$order_via','$ordername','$orderemail','$order_phone','$order_address','$createdby','$riderid')") or die(mysqli_error($link));
										if(mysqli_affected_rows($link))
	                                  {
	                                  	$row_array["status"]=1;
