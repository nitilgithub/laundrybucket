...
$return_arr = array();
 ...
 $uid=mysqli_real_escape_string($link,$_GET["uid"]);
	...
	$check_query="select * from tblusers WHERE UserId='$uid'";
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		if($row['UserPhone']!=NULL&&$row['UserPhone']!="")
		{
		$loginid=$row['UserPhone'];
		}
		else {
			$loginid=$row['UserEmail'];
		}
		
		$q2="update tblusers SET UserVerificationCode='$otp' where UserId='$uid'";
