...
$remarksbyid=intval($_GET["uid"]);
 	...
 	if(empty($pickdate) || ($picktime==-1) || ($pickup_address==-1)||($city==-1))
	{
		$row_array["status"]=0;
	...
	$data=mysqli_fetch_array($r);
	 ...
	 else 
	 {
		 ...
		 */
		  ...
		  $result=mysqli_query($link,"insert into tbl_orders(OrderUserId,Order_PickDate,Order_Picktime,PickupAddress,OrderCity,OrderStatusId,Order_Via,CreatedBy,Remarks,addon)
		  values('$uid','$pickdate','$picktime','$pickup_address','$city','$order_status_id','$order_via','$createdby','$remarks',NOW())") or die(mysqli_error($link));
					...
					if(mysqli_affected_rows($link))
	                 {
							...
							$usermessage = str_replace('%pickupaddress%', $pickup_address, $usermessage);
							$usermessage = str_replace('%remarks%', $remarks, $usermessage);							
							...
							//$adminmessage = str_replace('%deliverytype%', $delivery_type, $adminmessage);
							$adminmessage = str_replace('%pickdate%', $pickdate, $adminmessage);
							$adminmessage = str_replace('%picktime%', $picktime, $adminmessage);
							$adminmessage = str_replace('%pickupaddress%', $pickup_address, $adminmessage);							
		  					$adminmessage = str_replace('%remarks%', $remarks, $adminmessage);	
								...
								$ch = curl_init();
								$url= "http://alerts.kapsystem.com/api/web2sms.php?workingkey=A28560483f0aa800b63f2d7ddeb3acb5a&to=$client_phone,7876786000&sender=BUCKET&message=$txtmsg";
