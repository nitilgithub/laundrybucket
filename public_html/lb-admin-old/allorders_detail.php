...
function update_orderitemdetail($order_id,$receipt_id,$order_totalamount,$filepath,$wallet_deduct_amt,$order_status,$ord_remarks)  //update_orderitemdetail($order_id,$receipt_id,$order_weight,$filepath,$user_subsid)
{
	...
	$result=mysql_query("update tbl_orders set OrderReceiptId='$receipt_id',OrderTotalAmount='$order_totalamount',OrderReceiptPic='$filepath',walletdeduction_amt='$wallet_deduct_amt',OrderStatusId='$order_status',Review='$ord_remarks' where OrderId='$order_id'");
	 if($result)
  {
                            ...
                            <div class="x_panel">
                                ...
                                <?php
											...
											<tr class="even pointer">
											   ...
											   {
											   	?>
											  	<td> &#8377; <?php echo $order_total_amt; ?> </td>
											   	<?php
											   }
											   else {
 ...
 <!-- End Section for Order Detail -->  
    ...
    ?>
		...
		if(isset($_POST["btnadditems"]))
		{
			...
			$order_totalamount=mysql_real_escape_string($_POST["ordertotalamount"]);
   ...
   if($filename=="")
		...
		$filepath=$_POST['hidephoto'];
			...
			$filepath="images/all-orderitem-receipt/".date("m-d-y").'_'.time().'.'.$extension["extension"];
				...
				$result3=mysql_query("SELECT * FROM tbl_reward") or die(mysql_error());
				...
				$result4=mysql_query("SELECT ifnull(SUM(amount),0) as wta FROM tbl_wallet WHERE uid='$uid'") or die(mysql_error());
			...
			if($wallet_total_amt==0) //have no money in wallet
			{
				...
				$result5=mysql_query("SELECT ifnull(SUM(walletdeduction_amt),0) as wda FROM tbl_orders WHERE (OrderUserId='$uid'&& OrderId!='$order_id')") or die(mysql_error());
			if(mysql_num_rows($result5)>0)
			{	
				$row5=mysql_fetch_array($result5);
				$getwalletdeduct_amt=$row5["wda"];
				...
				if($final_wallet_total_amt>0)
				{
					...
					$wallet_discount=($order_totalamount*$fixed_walletdeduction_amt)/100;
		...
		if(!empty($ord_newremarks))
		{
			...
			{
				update_orderitemdetail($order_id,$receipt_id,$order_totalamount,$filepath,$wallet_deduct_amt,$order_status,$ord_remarks);
			}  
		}
		else {
		...
		?>
                                    ...
                                    <form method="post" role="form" class="form-horizontal" enctype="multipart/form-data">
                                     ...
                                     -->
                                      ...
                                      <div class="item form-group" id="qty">
                                            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="itname">Order Total Amount (Rs)
                                            </label>
                                            <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input type="text" name="ordertotalamount" pattern="^-?\d+(?:\.\d+)?$" title="Please enter digit only" id="ordertotalamount" value="<?php echo $order_total_amt; ?>" class="form-control col-md-7 col-xs-12" placeholder=""/>
                                        &nbsp;<span id="errmsg" class="text-warning" style="font-size:1.4em"></span>
...
<!--Start of Dryclean Order status Edit -->
	...
	if(mysql_affected_rows())
	{
		$usphone=empty($row1["UserPhone"])?$uphone : $row1["UserPhone"];
		
		
		 $result23=mysql_query("SELECT * from tbl_orderstatus_id where order_status_id='$orstatus'");			
		if(mysql_num_rows($result23)>0)
		 {
			 $row23=mysql_fetch_array($result23);
			 $orstatustext=$row23["order_status_text"];
		
		
		if(!empty($usphone))
		{
				$txtmsg=urlencode("Your Laundry Bucket order status for order id- $order_id has been updated to $orstatustext");
				$ch = curl_init();
