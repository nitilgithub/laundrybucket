...
$result=mysqli_query($conn,$query);
	...
	{
		$result=mysql_query("select * from tbl_orders where(OrderStatusId!=5) and franchiseId='".$_SESSION['loginid']."' order by OrderId DESC");
	}			
	else {
		
	$result=mysql_query("select * from tbl_orders where(OrderStatusId!=5) order by OrderId DESC");
	}
   ...
   $result1=mysql_query("select * from tblusers where UserId='$userid'");

	$row1=mysql_fetch_array($result1);
	
	//$row_array['phpne']="".$row1['UserPhone']."";
	
	$result2=mysql_query("select * from tbl_employee where empId='$riderid'");
	...
	$row_array["order_email"]=$row1["UserEmail"]."<br>".$row1['UserPhone'];
