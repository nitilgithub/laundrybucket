...
$result=mysqli_query($conn,$query);
	...
	{
		$result=mysql_query("select * from tbl_orders where(OrderStatusId!=5) and franchiseId='".$_SESSION['loginid']."' and comboOfferId!='' order by OrderId DESC") or die(mysql_error());
	}			
	else {
		
	$result=mysql_query("select * from tbl_orders where(OrderStatusId!=5) and comboOfferId!='' order by OrderId DESC") or die(mysql_error());
	}
...
$cofferid=$row['comboOfferId'];
$select=mysql_query("select * from tbl_combo_offer where offerId='$cofferid'");
$selrow=mysql_fetch_array($select);
$startdate=$row['addon'];
$pvalidity=$selrow['purchaseValidity'];
...
if($days>$pvalidity){
	$row_array['validdays']=0;
}
else {
	$row_array['validdays']=1;
}
