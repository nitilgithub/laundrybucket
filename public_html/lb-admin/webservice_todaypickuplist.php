...
$result=mysqli_query($conn,$query);
 ...
 if($_SESSION['loginrole']==9)
	{
		$result=mysql_query("select * from tbl_orders where(OrderStatusId!=5) and (STR_TO_DATE(Order_PickDate, '%m/%d/%Y') = CURDATE() or Order_PickDate=CURDATE()) and franchiseId='".$_SESSION['loginid']."' order by OrderId DESC") or die(mysql_error());
	}			
	else {                                   
