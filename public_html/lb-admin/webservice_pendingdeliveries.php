...
{
	$res=mysql_query("select * from tbl_suborders where DeliveryDate<CURDATE() and STR_TO_DATE(DeliveryDate, '%m/%d/%Y') < CURDATE() and (DeliveryStatusId!=0 and DeliveryStatusId!=4) and OrderId in (select OrderId from tbl_orders where OrderStatusId!=5 and franchiseId='".$_SESSION['loginid']."')");
}			
else {
	
$res=mysql_query("select * from tbl_suborders where DeliveryDate<CURDATE() and STR_TO_DATE(DeliveryDate, '%m/%d/%Y') < CURDATE() and (DeliveryStatusId!=0 and DeliveryStatusId!=4) and OrderId in (select OrderId from tbl_orders where OrderStatusId!=5)");
