...
$result=mysqli_query($conn,$query);
	...
	$result=mysql_query("select * from tbl_orders where(OrderStatusId!=5) order by OrderId DESC limit 0,$vl") or die(mysql_error());
...
while($q2=mysql_fetch_array($q1))
{
	...
	$ordertypes=$row_array['ordertypes'];
	
	$orderdetail="Id: ".$row["OrderId"]."<br>".$ordertypes."<br>Pickup: ".date("Y-m-d", strtotime($pickupdate))."<br>Amount: ".$row_array['order_total_amt_weight']."<br>".$data2['order_status_text']."<br>RNo: ".$row['OrderReceiptId']."<br>Payment: ".$row_array['payment_status'];
	
	$clientdetail=$row1["UserFirstName"]." ".$row1["UserLastName"]."<br>".$row1["UserEmail"]."<br>".$row1["UserPhone"]."<br>".$row["PickupAddress"].", ".$row["OrderCity"];
