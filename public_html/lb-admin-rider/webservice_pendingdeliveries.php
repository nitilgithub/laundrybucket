...
$res=mysql_query("select * from tbl_suborders where DeliveryDate<CURDATE() and STR_TO_DATE(DeliveryDate, '%m/%d/%Y') < CURDATE() and DeliveryStatusId=3 and OrderId in (select OrderId from tbl_orders where OrderStatusId!=5)");
...
$orderdetail="Id: ".$row["OrderId"]."<br>SubOrder: ".$row["SubOrderId"]."<br>Delivery: ".$deliverydate."<br>Type: ".$r["ServiceName"]."<br>Amount: ".$row_array['order_total_amt_weight']."<br>".$data2['order_status_text']."<br>Payment: ".$row_array['payment_status'];

$clientdetail=$row1["UserFirstName"]." ".$row1["UserLastName"]."<br>".$row1["UserEmail"]."<br>".$row1["UserPhone"]."<br>".$row["DeliveryAddress"].", ".$rw["OrderCity"];

$row_array['order_detail']=$orderdetail."<br>".$clientdetail;
